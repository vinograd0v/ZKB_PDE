...
\textcolor{blue}{Preguntar pq es en todo R(typo?)}\\
...
\begin{definition}
    Dado $s\geq 0.$ Diremos que el problema esta localmente bien planteado en $H^s(\T^n)$ si:
    \begin{itemize}
        \item \textbf{(Existencia y Unicidad).} Dado $u_0\in H_s(\T^n),$ existe $T>0$ y una única solución de la formula de Duhamel con dato inicial $u_0$ en el espacio $C([0,T];H^s(\T^n)).$
        \item \textbf{(Dependencia Continua).} Dado $u_0\in H^s(\T^n)$ existe una vecindad $V$ de $u_0$ en $H^s(\T^n)$ y $T>0$ tal que la aplicación dato inicial solución $v_0\in H^s(\T^n)\to v\in C([0,T];H^s(\T^n)) $ es continua.
    \end{itemize}
\end{definition}