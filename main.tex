...
Sean $f,g\in H^s(\T^n)$, como $\widehat{f}, \widehat{g} \in l^1(\mathbb{Z}^n) \cap \ell^2(\mathbb{Z}^n)$, por la desigualdad de Young tenemos que $\widehat{f} * \widehat{g} \in \ell^2(\mathbb{Z}^n)$, esto nos dice que el producto $f g=(\widehat{f} * \widehat{g})^{\vee}$ define una función en $L^2([-\pi, \pi])$. Además, se tiene que $\widehat{f g}=\widehat{f} * \widehat{g}$, Así:
