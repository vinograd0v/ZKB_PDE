...
\maketitle
\begin{abstract}
Este trabajo se centra en el estudio de la ecuación de Zakharov-Kusnetsov-Burgers modificada en $\T^n$, es cual es un dominio periódico. Utilizando la serie de Fourier y espacios de Sobolev, se realizará la buena colocación local del problema en dichos espacios. El resultado principal demuestra que, para $n \geq 1$ y $s > \frac{n}{2}$, existe una solución única y local en el tiempo que depende del dato inicial, adicionalmente, se realizará una demostración en base a la fórmula integral de Duhamel, lo que permite un análisis riguroso de la existencia, unicidad y dependencia continua de las soluciones en los espacios $ H^s(T^n)$.
\end{abstract}
...
Consideremos el problema de valor inicial asociado a la \textit{ecuación con no linealidad modificada de Zakharov-Kustnesov-Burgers}
$$\begin{cases}
